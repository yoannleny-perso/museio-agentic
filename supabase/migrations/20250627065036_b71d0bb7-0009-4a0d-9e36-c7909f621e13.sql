...
-- Delete all data for users ivan.guelton@audaciangroup.com and ivan.guelton@icloud.com
-- This must be done in the correct order to respect foreign key constraints

BEGIN;

-- First, let's identify the user IDs and verify they exist
DO $$
DECLARE
    user1_id UUID;
    user2_id UUID;
    user1_email TEXT := 'ivan.guelton@audaciangroup.com';
    user2_email TEXT := 'ivan.guelton@icloud.com';
...
BEGIN
    ...
    user1_exists := (user1_id IS NOT NULL);
    user2_exists := (user2_id IS NOT NULL);
    
    IF user1_exists THEN
        RAISE NOTICE 'Found user 1: % (ID: %)', user1_email, user1_id;
    ELSE
        RAISE NOTICE 'User 1 not found: %', user1_email;
    END IF;
    
    IF user2_exists THEN
        RAISE NOTICE 'Found user 2: % (ID: %)', user2_email, user2_id;
    ELSE
        RAISE NOTICE 'User 2 not found: %', user2_email;
    END IF;
    
    -- Only proceed if at least one user exists
    IF NOT user1_exists AND NOT user2_exists THEN
        RAISE EXCEPTION 'Neither user found in the database';
    END IF;
END $$;
...
WHERE user_id IN (
    ...
    WHERE email IN ('ivan.guelton@audaciangroup.com', 'ivan.guelton@icloud.com')
);
...
BEGIN
    ...
    SELECT COUNT(*) INTO remaining_records FROM (
        SELECT user_id FROM public.sent_invoices WHERE user_id IN (
            SELECT id FROM auth.users WHERE email IN (user1_email, user2_email)
        )
        UNION ALL
    ...
    RAISE NOTICE 'Successfully deleted all data for users: % and %', user1_email, user2_email;
END $$;
