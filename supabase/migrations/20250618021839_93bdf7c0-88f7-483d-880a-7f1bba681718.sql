...
-- COMPREHENSIVE RLS POLICY OPTIMIZATION - Fix Performance Warnings and Duplicates
-- This migration optimizes auth function calls and eliminates ALL duplicate policies

-- ===============================
-- DROP ALL EXISTING RLS POLICIES - COMPLETE CLEANUP
-- ===============================

-- Drop ALL possible policy variations on user_onboarding (5+ policies)
...
-- Drop ALL possible policy variations on sent_invoices (6+ policies)
...
-- Drop ALL possible policy variations on user_invoice_sequences (5+ policies including problematic ALL)
...
DROP POLICY IF EXISTS "Users can manage their own invoice sequences" ON public.user_invoice_sequences;
...
-- Drop ALL possible policy variations on profiles (4+ policies including unnecessary DELETE)
...
-- ===============================
-- CREATE OPTIMIZED POLICIES - MAXIMUM PERFORMANCE
-- Using (SELECT auth.uid()) for single evaluation per query (not per row)
-- Exactly ONE policy per operation per table - NO duplicates
-- ===============================

-- PROFILES TABLE - 3 policies (no DELETE - profiles shouldn't be deletable)
...
-- JOBS TABLE - 4 policies (OPTIMIZED - fixes the performance warning)
...
-- SENT_INVOICES TABLE - 4 policies (was 6+, now exactly 4)
...
-- USER_ONBOARDING TABLE - 4 policies (was 5+, now exactly 4)
...
-- USER_INVOICE_SEQUENCES TABLE - 4 policies (was 5+ with problematic ALL, now exactly 4 individual)
